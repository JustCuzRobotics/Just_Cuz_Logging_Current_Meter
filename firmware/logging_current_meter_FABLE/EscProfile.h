 ...
 * microseconds; a 0 ms ramp is an instant step. PRE and POST (Log Test only)
 * and ARMING sit at idle.
 *
...
#define ESC_TEST_PRE_MS    3000     /* Log Test pre-roll at idle          */
...
enum EscPhase : uint8_t {
  ...
  PH_ARMING,      /* 2 s idle hold before a cycle starts                     */
  PH_PRE,         /* Log Test pre-roll                                       */
...
struct EscProfile {
  ...
  uint16_t rampUpMs, dwellHiMs, rampDnMs, dwellLoMs;
};
 ...
 * PH_OFF / PH_MANUAL / PH_ARMING are not timed here. */
static inline uint32_t profilePhaseMs(uint8_t ph, const EscProfile &p) {
  switch (ph) {
    case PH_PRE:      return ESC_TEST_PRE_MS;
...
static inline uint16_t profilePulse(uint8_t ph, uint32_t elapsedMs, const EscProfile &p,
  ...
  switch (ph) {
    ...
    default:          return escTypeIdleUs(type);   /* ARMING, PRE, POST, MANUAL idle */
  }
}
